...
app.get("/", (_req, res) => {
  res.send("FrostRoute Backend Running");
});
...
/**
 * Run Dijkstra (C executable) asynchronously.
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!fs.existsSync(exe)) {
      return resolve({
        ok: false,
        error:
          "Dijkstra binary not found. Run: npm run build:c (requires gcc in PATH) from the backend folder.",
      });
    }
    ...
    child.on("close", (code) => {
      if (code !== 0) {
        return resolve({
          ok: false,
          error: stderrData || stdoutData || "dijkstra exited with error",
        });
      }
      ...
      try {
        ...
        resolve({
          ok: false,
          error: "Invalid JSON from dijkstra",
          raw: stdoutData,
        });
      }
    });
...
app.listen(PORT, () => {
  console.log(`FrostRoute API running on port ${PORT}`);
});
